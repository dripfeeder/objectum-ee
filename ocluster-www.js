	...
	if (!config.storages [storageCode]) {
		...
		if (pathname.substr (0, 6) == "/files") {
			...
			res.sendfile (filePath, function (err) {
				if (err) {
					console.log (err);
					res.end ("{header: {error: " + JSON.stringify (err) + "}}");
				};
			});
			*/
...
//   	if (request.query.authorize == 1 || (request.url.indexOf ("/plugins/") > -1 && !sessionId)) {
   	if (!sessionId) {
	...
	var req = http.request ({
		...
		res.on ("end", function () {
		   	if (request.query.authorize == 1 && data) {
				...
				if (tokens.length && tokens [0].length == 40) {
